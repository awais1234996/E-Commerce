...
// if (isset($_POST['submit'])) {
    ...
    if (empty($fname) || empty($lname) || empty($email) || empty($phone) || empty($country) || empty($state) || empty($city) || empty($pcode) || empty($address1) || empty($address2) || empty($password) || empty($cpassword)) {
        ...
        // $emailrun = mysqli_query($conn, $emailsql);
        if ($emailsql) {
            echo 2;
        } else {
            if ($password == $cpassword) {
                $usersql = $obj->insert("user",["fname"=>$fname,"lname"=>$lname,"email"=>$email,"phone"=>$phone,"country"=>$country,"state"=>$state,"city"=>$city,"pcode"=>$pcode,"address1"=>$address1,"address2"=>$address2,"password"=>$password,"cpassword"=>$cpassword,]);
                            if ($usersql) {
                    echo 3;
                } else {
                    echo 4;
